...
# 🌀 Alpha Maze: Visual thinker

<BlogAuthors authors={["Alan Dao", "Bach Vu", "Louis Le", "Rex Ha"]}/> 

## Introduction
Recent breakthroughs in large language models (LLMs) have demonstrated a fascinating concept: giving models time to think dramatically improves their performance. These "reasoning" models, as they're often called, have achieved remarkable results on complex mathematical benchmarks like [AIME and MATH](cite AIME, MATH). This success isn't limited to massive models like O3-mini and Deepseek-r1; the community has also achieved strong results by fine-tuning smaller, "distilled" models, such as [1.5b](cite deepsclaer) and [7b](cite open-r1) versions.

The core algorithm driving much of this progress is [GRPO (Generalized Reinforcement Learning with Policy Optimization)](cite deepseek-r1). GRPO is an online learning method, meaning it constantly refines the model using data generated during the training process itself. It's a powerful way to iteratively improve performance.
...
With the same spririt of the arc-agi test, we make the model learn on maze solver which is also the same easy for human but hard for AI. We're exploring how to enhance the visual thinking capabilities of reasoning models by focusing on a specific, visually-driven task: maze solving.
...
*The video demonstrates Alpha Maze's process: inputting a maze, the model will reason through and output of the most likely direction.*

## Overview

Prior research, such as Microsoft's "Multimodal Visualization-of-Thought (MVoT)", has shown that models can solve image-based mazes by generating visual representations of their reasoning process. However, these models still have limitations in their ability to create accurate visual representations and are often evaluated using multiple-choice question (MCQ) setups, which may not fully capture their reasoning abilities.

Alpha Maze takes a different approach. We built our model on a distilled, 7-billion parameter Qwen model (based on Deepseek), and trained it to solve mazes presented in a text-based format. Our core hypothesis is that if a model can successfully reconstruct a maze from a text description and use that internal representation to plan its next move, it demonstrates a form of visual reasoning.
...
This system allows us to create step-by-step navigation sequences. Each step represents the entire maze state, with a single directional indicator, making it ideal for training a model to reason about its movements.

Outcome we obtain 100,000 maze examples link: https://huggingface.co/datasets/jan-hq/Maze-Reasoning.
...
**SFT Hyperparameters:** 
Training data: 75,000 samples cut off since we only train on 4096 maxlength.
Here’s the table without the category section:  
...
```
Total Reward = correctness_reward + int_reward + strict_format_reward + soft_format_reward + xmlcount_reward
```
...
*   **Base Model:** [DeepSeek R1 Distill Qwen 7B](https://huggingface.co/deepseek-ai/DeepSeek-R1-Distill-Qwen-7B/tree/main).
...
| :----- | :--------- | :------------------------------------------------------------------------------------- | :--------------------------------------------------------------------- | :------------ | :--------- | :---- | :------ | :----------------------- |
| exp-1  | 2025-02-06 | [Full-Finetune](https://github.com/janhq/visual-thinker/blob/main/training/Llama-factory-config/Qwen2.5_1.5B_distil.yaml) | [Maze Reasoning](https://huggingface.co/datasets/jan-hq/Maze-Reasoning) | 1e-5          | 32         |       | 0.01    | ~1.5 hours on 6xA6000s |
| exp-2  | 2025-02-06 | [Full-Finetune](https://github.com/janhq/visual-thinker/blob/main/training/Llama-factory-config/Qwen2.5_1.5B_distil.yaml) | [Maze Reasoning](https://huggingface.co/datasets/jan-hq/Maze-Reasoning) | 1e-5          | 32         |       | 0.01    | ~1.5 hours on 6xA6000s |
| exp-3  | 2025-02-08 | [Full-Finetune](https://github.com/janhq/visual-thinker/blob/main/training/Llama-factory-config/Qwen2.5_7B_distil.yaml)  | [Maze Reasoning](https://huggingface.co/datasets/jan-hq/Maze-Reasoning) | 1e-5          | 32         |       | 0.0077  | ~12 hours on 6xA6000s  |
...
## Appendix
prompt để instruction model cho trainign SFT and GRPO
```
